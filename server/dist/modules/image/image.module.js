"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ImageModule = void 0;
const common_1 = require("@nestjs/common");
const image_service_1 = require("./image.service");
const image_controller_1 = require("./image.controller");
const image_repository_1 = require("./image.repository");
const s3_manager_module_1 = require("../s3-manager/s3-manager.module");
const student_module_1 = require("../student/student.module");
const superviser_module_1 = require("../superviser/superviser.module");
const coordinator_module_1 = require("../coordinator/coordinator.module");
const application_module_1 = require("../application/application.module");
let ImageModule = class ImageModule {
};
exports.ImageModule = ImageModule;
exports.ImageModule = ImageModule = __decorate([
    (0, common_1.Module)({
        imports: [student_module_1.StudentModule, superviser_module_1.SuperviserModule, coordinator_module_1.CoordinatorModule, application_module_1.ApplicationModule, s3_manager_module_1.S3ManagerModule],
        providers: [image_service_1.ImageService, image_repository_1.ImageRepository],
        controllers: [image_controller_1.ImageController]
    })
], ImageModule);
//# sourceMappingURL=image.module.js.map